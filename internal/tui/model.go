...
func (m Model) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
	...
	case extractDoneMsg:
		...
		// Store response and show quality selection.
		...
		// If only one quality, auto-select it.
		if len(m.availableQualities) == 1 {
			m.selectedQuality = m.availableQualities[0]
			link := m.selectedQuality.Links[0].Src
			m.state = viewPlaying
			m.status = fmt.Sprintf("%s [%s]", m.selectedQuality.Label, linkType(m.selectedQuality.Links[0].Type))
			return m, m.startPlay(link, m.selectedQuality.Label)
		}
		m.state = viewQuality
		m.cursor = 0
		return m, nil
...
func (m Model) startPlay(url string, quality string) tea.Cmd {
	return func() tea.Msg {
		title := fmt.Sprintf("%s — Серия %d [%s] (%s)",
			m.animeTitle,
...
func (m Model) handleQualityKey(msg tea.KeyMsg) (tea.Model, tea.Cmd) {
	...
	case "down", "j":
		if m.cursor < len(m.availableQualities)-1 {
			m.cursor++
		}
	case "enter":
		q := m.availableQualities[m.cursor]
		link := q.Links[0].Src
		m.selectedQuality = q
		m.state = viewPlaying
		m.status = fmt.Sprintf("%s [%s]", q.Label, linkType(q.Links[0].Type))
		return m, m.startPlay(link, q.Label)
	case "q", "esc":
		m.state = viewEpisodes
		m.kodikResp = nil
		m.availableQualities = nil
...
// linkType returns a human-readable video codec/format string.
