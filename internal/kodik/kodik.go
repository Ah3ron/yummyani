...
// AvailableQualities returns the list of qualities that have at least one link,
// ordered from highest to lowest.
func AvailableQualities(resp *Response) []Quality {
	var result []Quality
	for _, q := range []Quality{
		{"1080p", resp.Links.Quality1080},
		{"720p", resp.Links.Quality720},
		{"480p", resp.Links.Quality480},
		{"360p", resp.Links.Quality360},
	} {
		if len(q.Links) > 0 {
...
func BestLink(resp *Response) string {
	for _, links := range [][]Link{resp.Links.Quality1080, resp.Links.Quality720, resp.Links.Quality480, resp.Links.Quality360} {
		if len(links) > 0 {
			return links[0].Src
		}
	}
...
func decodeLinks(resp *Response) {
	...
	// 1080p: derive from 360p if available, otherwise decode independently.
	for i := range resp.Links.Quality1080 {
		if base360 != "" {
			resp.Links.Quality1080[i].Src = strings.Replace(base360, "/360.mp4", "/1080.mp4", 1)
		} else {
			resp.Links.Quality1080[i].Src = decodeLink(resp.Links.Quality1080[i].Src)
		}
	}
}
